...
## 4. 아래 내용 `monitorio.sh` 파일의 `echo "Enter standby"` 위에 추가하고 저장
  ```sh
...
## 5. `standby.conf` 파일을 수정하고 저장
* `standby_time`에 원하는 시간(분) 입력

## 6. reboot 후 아래 명령어 `전체`를 복사하고 실행
  ```sh
